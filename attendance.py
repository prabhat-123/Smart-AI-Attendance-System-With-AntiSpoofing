...
def login():
    ...
    ######################################### Function to recognize the face
                ...
                def getall_staffs():
                    conn = pymysql.connect(host = "localhost", user = "root", password = "", database = "recognition")
                    cur = conn.cursor()
                    cur.execute("select fname,email_address from attendance")
                    data = cur.fetchall()
                    conn.close()
                    all_staffs = {}
                    if len(data) != 0:
                        for(name,email) in data:
                            all_staffs[name] = email
                    return all_staffs
                ...
                def registered_vs_absent_staffs(all_staffs):
                    ...
                    # end_minute = 0
                    cur = conn.cursor()
                    cur.execute("select name,time from report where date=%s ",(date))
                    output = cur.fetchall()
                    if len(output)!= 0:
                        registered_staff_names = []
                        for(x,y) in output:
                            y_hour = y.split(":")[0]
                            y_minute = y.split(":")[1]
                            if(int(y_hour) >= start_hour and int(y_hour) <= end_hour):
                                registered_staff_names.append(x)
                        absent_staff_names = []
                        all_staff_names = list(all_staffs.keys())
                        for x in all_staff_names:
                            if x not in registered_staff_names:
                                absent_staff_names.append(x)
                    return absent_staff_names
                
                def absent_emails():
                    all_staffs = getall_staffs()
                    absent_staff_names = registered_vs_absent_staffs(all_staffs)
                    absent_staff_emails = []
                    for item in absent_staff_names:
                ...
                # schedule.every().day.at("12:12").do(job)
                ...
                def face_recognize():
                    print("[INFO] starting video stream...")
                    training_obj = Training(embedding_path='models/embeddings.pickle')
                    [name_label,id_label,Embeddings,names,face_ids] = training_obj.load_embeddings_and_labels()
                    ...
                    while len(predictions) <= 10:
                        try:
                            ...
                            for (x,y,w,h) in faces:  
                                ...
                                COLORS = np.random.randint(0, 255, size=(len(name_label.classes_), 3), dtype="uint8")
                                # perform classification to recognize the face
                                preds = recognizer.predict_proba(embedding)[0]
                                p = np.argmax(preds)
                                proba = preds[p]
                                name = name_label.classes_[p]
                                face_id = id_label.classes_[p]
                                print(name)
                                print(face_id)
                ...
                ############### Function to recognize the face
                    ...
                    # # end_minute = 30
                    # dict = getall_staffs()
                    # email = dict[final_name]

                    # cur1 = conn.cursor()
                    # cur1.execute("select name,time from report where name = %s and date = %s",(final_name,date))
                    # data = cur1.fetchall()
                    # if len(data)!= 0:
                    ...
                    #         cur2 = conn.cursor()
                    #         cur2.execute("insert into report(name,date,time,status,email) VALUES (%s,%s,%s,%s,%s)", (
                    #                                                                                             final_name,
                    #                                                                                             date,
                    #                                                                                             time,
                    #                                                                                             status,
                ...
                def report():
                    ...
                    def save_data():
                        delete()
                        mydb = pymysql.connect(host = 'localhost', user = 'root', password = '', database = 'recognition')
                        with open('Attendance_Details/attendance.csv') as csv_file:
                        ...
                        query = "insert into report values(%s, %s, %s, %s)"
                    ...
                    search_today = Button(text_fill, text = "Visualize", font = ("times new roman", 15, "bold"), command = visualize, width = 15)
                    ...
                    report_table = Treeview(table_frame, columns = ("Date", "Name", "Time", "Status"), xscrollcommand = scroll_x.set, yscrollcommand = scroll_y.set)
                    scroll_x.pack(side = BOTTOM, fill = X )
                    scroll_y.pack(side = RIGHT, fill = Y)
                    scroll_x.config( command = report_table.xview)
                    scroll_y.config( command = report_table.yview)  
                    report_table.heading('Date', text ="")
                    report_table.heading('Name', text = "")
                    report_table.heading("Time", text= "")
                    report_table.heading("Status", text = "")
                    report_table['show'] = 'headings'
                    report_table.column("Date",  width = 178)
                    report_table.column("Name", width = 179)
                    report_table.column("Time", width = 179)
                    report_table.column("Status", width = 178)
