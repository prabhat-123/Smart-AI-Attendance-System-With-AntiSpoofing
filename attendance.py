...
try:
    ...
    face_cascade = cv2.CascadeClassifier("C:/Users/ASUS/Anaconda3/Lib/site-packages/cv2/data/haarcascade_frontalface_default.xml")
...
def login():
                                        ...
                                        if(re.search('^[9]\d{9}$', contact_var.get())):
                                            ...
                                            if(re.search(regex, address_var.get())):
                                                ...
                                                if id in staff_ids:
                                                    ...
                                                    if (q == True):
                                                        input_directory = os.path.join(dataset_dir,staff_name + "_" + id)
                                                        shutil.rmtree(input_directory)
                                                        os.makedirs(os.path.join(dataset_dir,name + "_" + id), exist_ok = 'True')
                                                        count = 1
                                                        print("[INFO] starting video stream...")
                                                        video_capture = cv2.VideoCapture(0)
                                                        while count <= 50:
    ...
    ######################################### Function to recognize the face
                ...
                def face_embedding():
                    ...
                    backgrd.pack()
                    embed_title = Label(fe, text = "Extract And Save Embeddings", fg = 'white',font = ("times new roman", 30, "bold"), bg = "brown")
