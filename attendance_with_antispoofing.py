...
def login():
    ...
    ######################################### Function to recognize the face
                        ...
                        while len(predictions) <= 10:
                            try:
                                ...
                                for (x,y,w,h) in faces:  
                                    ...
                                    processed_face = img_to_array(resized_face)
                ...
                def report():
                    ...
                    #     conn = pymysql.connect(host = 'localhost', user = 'root', password = '', database = 'recognition')
                    #     cur = conn.cursor()
                    #     cur.execute("insert into report(id, date, name, time, status) values(%s, %s, %s, %s, %s)")
                    #     conn.commit()
                    #     conn.close()
                    ...
                    #     mydata = cur.fetchall()
                    #     if len(mydata) < 1:
                    #         messagebox.showerror( ‘Error’, "No Data Available", parent = report)        
                    #         # path = os.path.join(root_dir,'Attendance_Details')
                    #     else:
                    ...
                    # def export_csv():
                    #     report_dir = os.path.join(root_dir,'Attendance_Details')
                    #     conn = pymysql.connect(host = 'localhost', user = 'root', password ='', database = 'recognition' )
                    #     answer = simpledialog.askstring( 'Input', 'Enter the Required Date to Save, 
                    #     parent = report)
                    #     if answer i
                    #     messagebox.showinfo( ‘Information’, ‘Please Enter the Data’)
                    #     else:
                    #     cur = conn.cursor()
                    #     cur.execute(‘select * from report where date = %s ', (answer))
                    #     mydata = cur.fetchall()
                    #     if len(mydata) < 1:
                    #     messagebox.showerror( ‘Error’, "No Data Available", parent = report)        
                    #     path = os.path.join(root_dir,'Attendance_Details')
                    #     fln = filedialog.asksaveasfilename(initialdir = path, title = ‘Save CSv', filetype = (("CSV File”,
                    #     with open(fln, mode = ‘w') as myfil
                    #     exp_uriter = csv.uriter(myfile, delimiter = *,")
                    #     for i in mydata:
                    #     exp_writer wuriterow(i)
                    #     messagebox.showinfo( "Success", "Your data has been exported to "+os.path.basename(fln)+" successfully.”, parent
                        # conn. commit()
                        # conn.close()
